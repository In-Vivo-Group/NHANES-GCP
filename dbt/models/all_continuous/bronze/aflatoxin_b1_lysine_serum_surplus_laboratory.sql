...
CASE
WHEN SSAFB IS NULL THEN NULL 
ELSE SAFE_CAST(SSAFB AS FLOAT64) 
 END as aflatoxin_b1_lysine_result_ng_ml, 

CASE
WHEN SAFE_CAST(ROUND(SAFE_CAST(SSAFBLC AS FLOAT64),0) AS INT64) = SAFE_CAST(ROUND(SAFE_CAST(0 AS FLOAT64),0) AS INT64) THEN 'detectable result' -- categorize numeric values
WHEN SAFE_CAST(ROUND(SAFE_CAST(SSAFBLC AS FLOAT64),0) AS INT64) = SAFE_CAST(ROUND(SAFE_CAST(1 AS FLOAT64),0) AS INT64) THEN 'below detectable limit' -- categorize numeric values
WHEN SSAFBLC IS NULL THEN NULL 
ELSE SAFE_CAST(SSAFBLC AS STRING) 
 END as aflatoxin_b1_lysine_result_comment_code, 
...
doc_file_url,
dataset,
 FROM {{ ref('stg_aflatoxin_b1_lysine_serum_surplus_laboratory') }}
