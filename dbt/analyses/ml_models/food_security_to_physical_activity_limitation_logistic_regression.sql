CREATE OR REPLACE MODEL `nhanes.physical_activity_limitation_food_security_logistic_regression`
OPTIONS(model_type='LOGISTIC_REG', 
input_label_cols=['physical_activity_limitation'], 
...
AND b.walking_for_a_quarter_mile_difficulty IS NOT NULL
...
CASE
WHEN IFNULL(walking_for_a_quarter_mile_is_difficult,0) + IFNULL(walking_up_ten_steps_is_difficult,0)
+ IFNULL(stooping_crouching_kneeling_is_difficult,0) + IFNULL(lifting_or_carrying_is_difficult,0)
+ IFNULL(walking_between_rooms_on_same_floor_is_difficult,0) + IFNULL(standingup_from_armless_chair_is_difficult,0) > 1 THEN 1
ELSE 0 END as physical_activity_limitation,
FROM initial_data;